...
{
    internal class DecimalElementParser : IFlowtideXmlParser
    {
        public async ValueTask<IDataValue> Parse(XmlReader reader)
        {
            ...
            switch (reader.NodeType)
            {
                case XmlNodeType.Text:
                    if (decimal.TryParse(reader.Value, CultureInfo.InvariantCulture, out var i))
                    {
                        returnVal = new DecimalValue(i);
                    }
                    else
                    {
