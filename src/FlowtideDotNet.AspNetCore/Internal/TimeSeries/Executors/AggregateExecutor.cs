...
{
    internal abstract class AggregateExecutor : IMetricExecutor
    {
        ...
        public AggregateExecutor(List<IMetricExecutor> series, IReadOnlyDictionary<string, string>? tags = default)
        {
            this.series = series;
            Tags = tags ?? ImmutableDictionary<string, string>.Empty;
        }

        protected abstract double AggregateValues(IEnumerable<double> values);
        ...
        public async IAsyncEnumerable<MetricResult> GetValues(long startTimestamp, long endTimestamp, int stepWidth)
        {
            ...
            while (!done.All(x => x))
            {
                ...
                if (outputIterators.Count > 0)
                {
                    yield return new MetricResult(AggregateValues(outputIterators.Select(x => x.Item1.Current.value)), lowestTimestamp);
                }
            }
        }
    }
