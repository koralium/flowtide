...
{
    internal class DecimalEncoder : IParquetEncoder
    {
        ...
        public DecimalEncoder(FieldPath path)
        {
            _path = path;
        ...
        public void ReadNextData(ref AddToColumnParquet addToColumnFunc)
        {
            ...
            if (val.HasValue)
            {
                addToColumnFunc.AddValue(new DecimalValue(val.Value));
            }
            else
            {
