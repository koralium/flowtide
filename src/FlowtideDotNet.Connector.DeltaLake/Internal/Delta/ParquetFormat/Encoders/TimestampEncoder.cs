...
{
    internal class TimestampEncoder : IParquetEncoder
    {
        private readonly FieldPath _path;
        private DataField? _dataField;
        private DataColumn? _dataColumn;
        private DateTime?[]? _arr;
        private int _dataIndex;
        private int _repititionIndex;

        public TimestampEncoder(FieldPath path)
        {
            _path = path;
        ...
        public async Task NewRowGroup(IParquetRowGroupReader rowGroupReader)
        {
            ...
            _arr = (DateTime?[])_dataColumn.Data;
            _repititionIndex = 0;
            _dataIndex = 0;
        }
        ...
        public void ReadNextData(ref AddToColumnParquet addToColumnFunc)
        {
            ...
            if (val.HasValue)
            {
                addToColumnFunc.AddValue(new TimestampTzValue(val.Value));
            }
            else
            {
