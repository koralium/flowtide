...
namespace FlowtideDotNet.Core.ColumnStore.DataColumns
{
    ...
    internal class UnionColumn : IDataColumn, IReadOnlyList<IDataValue>
    {
        ...
        private NativeList<sbyte> _typeList;
        ...
        internal UnionColumn(List<IDataColumn> columns, FlowtideMemory typeListMemory, FlowtideMemory offsetMemory, int count, IMemoryAllocator memoryAllocator)
        {
            ...
            _offsets = new IntList(offsetMemory, count);
...
#pragma warning restore RS0042
        ...
        private unsafe void InsertRangeFromUnionColumn(int index, UnionColumn other, int start, int count, in BitmapList validityList)
        {
            ...
            _offsets.InsertRangeFromTypeBasedAddition(index, other._offsets, start, count, _typeList.Span, new Span<int>(thisDifference, 127), other._typeList.Span, new Span<int>(otherDifference, 127), _valueColumns.Count, other._valueColumns.Count, _memoryAllocator);
            InsertTypeRangeFromMapped(index, in other._typeList, start, count, new Span<sbyte>(mappingTable, 127), other._valueColumns.Count);
        }

        /// <summary>
        /// Inserts type ids from another list, remapped through the mapping table.
        /// </summary>
        private unsafe void InsertTypeRangeFromMapped(int index, in NativeList<sbyte> other, int start, int count, Span<sbyte> mapping, int sourceTypeCount)
        {
