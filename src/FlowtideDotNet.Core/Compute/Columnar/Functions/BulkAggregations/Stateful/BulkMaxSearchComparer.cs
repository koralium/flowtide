...
namespace FlowtideDotNet.Core.Compute.Columnar.Functions.BulkAggregations.Stateful
{
    public class BulkMaxSearchComparer : IBplusTreeComparer<BulkGroupValueRowReference, BulkGroupValueKeyContainer>
    {
        ...
        public BulkMaxSearchComparer(int groupingKeyLength)
        {
        ...
        public bool SeekPreviousPageForValue => true;
        ...
        public int CompareTo(in BulkGroupValueRowReference key, in BulkGroupValueKeyContainer keyContainer, in int index)
        {
            ...
            // Grouping keys are equal. Force routing to the rightmost branch to find the maximum!
            return 1;
        }
        ...
        public int FindIndex(in BulkGroupValueRowReference key, in BulkGroupValueKeyContainer keyContainer)
        {
            ...
            // Since it is found, return the rightmost child index (End + 1) to route to the maximum value.
            return End + 1;
        }
            ...
            Span<int> lookupBuffer)
        {
            if (_groupingKeyLength == 0)
            {
                if (keyContainer.Count > 0)
                {
                    ...
                    {
                        lowerBounds[i] = keyContainer.Count; // high + 1
                        upperBounds[i] = keyContainer.Count - 1;
                    }
                }
                else
                {
            ...
            // For each search key, if grouping keys matched (lowerBounds[i] >= 0),
            // route to the rightmost child containing the maximum value by setting lowerBounds[i] to upperBounds[i] + 1.
            for (int i = 0; i < lowerBounds.Length; i++)
            {
                if (lowerBounds[i] >= 0)
                {
