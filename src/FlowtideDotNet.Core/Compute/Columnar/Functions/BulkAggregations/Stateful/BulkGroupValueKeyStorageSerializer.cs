...
namespace FlowtideDotNet.Core.Compute.Columnar.Functions.BulkAggregations.Stateful
{
    public class BulkGroupValueKeyStorageSerializer : IBPlusTreeKeySerializer<BulkGroupValueRowReference, BulkGroupValueKeyContainer>
    {
        ...
        public BulkGroupValueKeyStorageSerializer(int groupingKeyLength, IMemoryAllocator memoryAllocator)
        {
        ...
        public BulkGroupValueKeyContainer CreateEmpty()
        {
            return new BulkGroupValueKeyContainer(_groupingKeyLength, _memoryAllocator);
        }

        public BulkGroupValueKeyContainer Deserialize(ref SequenceReader<byte> reader)
        {
            var batchResult = _batchSerializer.Deserialize(ref reader, _memoryAllocator);
            return new BulkGroupValueKeyContainer(_groupingKeyLength, batchResult.EventBatch);
        }
        ...
        public void Serialize(in IBufferWriter<byte> writer, in BulkGroupValueKeyContainer values)
        {
