...
{
    internal class CustomImmutableSourceFactory<T> : AbstractConnectorSourceFactory, IConnectorTableProviderFactory
        where T : class
    {
        private readonly string tableName;
        private readonly Func<ReadRelation, ImmutableGenericDataSourceAsync<T>> dataSourceFunc;

        public CustomImmutableSourceFactory(string tableName, Func<ReadRelation, ImmutableGenericDataSourceAsync<T>> dataSourceFunc)
        {
        ...
        {
            return new ImmutableGenericReadOperator<T>(dataSourceFunc(readRelation), readRelation, dataflowBlockOptions);
        }
    }
}
