...
{
    /// <summary>
    /// Peer queue, never checkpointed, regenerated by replay after rollback.
    /// </summary>
    internal class SubstreamTarget : IExchangeTarget
    {
        ...
        // Stop barrier store time, the escape measures from it.
        ...
        /// <summary>
        /// Stop barrier fetched and acked, none stored, or escaped.
        /// </summary>
        public bool ReadyToStop => !_stopBarrierStored || (_stopBarrierFetched && _stopBarrierFetchAcked) || StopAlignmentEscaped();

        /// <summary>
        /// Stop barrier queued, draining; refuse a returning peer.
        /// </summary>
        public bool StopBarrierStored => _stopBarrierStored;

        /// <summary>
        /// Half drain timeout, unfetched barrier only; fetched waits for ack.
        /// </summary>
        private bool StopAlignmentEscaped()
        {
            ...
            {
                // Never silent, a stop never rolls back, queue lost.
        ...
        public async Task OnLockingEvent(ILockingEvent lockingEvent)
        {
            ...
            try
            {
                if (_stopBarrierStored)
                {
                    // Drain cycle, the peer needs only the first barrier.
                    return;
                }
                ...
                {
                    // Stamped before the volatile flag, published with it.
        ...
        public async ValueTask<bool> ReadData(List<SubstreamEventData> outputList, int maxCount)
        {
            ...
            try
            {
                ...
                while (_queue.Count > 0 && count < maxCount)
                {
                    ...
                    {
                        // Ends at the barrier, dequeue is destructive, reader decides first.
                        break;
                    }
                }
