...
{
    internal class ParallelSource<T> : ITargetBlock<T>, IReceivableSourceBlock<T>, IDebuggerDisplay
    {
        ...
        private Func<ILockingEvent, Task<ILockingEvent>> _onCheckpoint;
        private Action _checkpointSent;

        /// <summary>Initializes the <see cref="BufferBlock{T}"/>.</summary>
        public ParallelSource(Func<ILockingEvent, Task<ILockingEvent>> onCheckpoint, Action checkpointSent) :
            this(onCheckpoint, checkpointSent, DataflowBlockOptionsExtensions.DefaultDataflowBlockOptions)
        { }
        ...
        public ParallelSource(Func<ILockingEvent, Task<ILockingEvent>> onCheckpoint, Action checkpointSent, DataflowBlockOptions dataflowBlockOptions)
        {
            _onCheckpoint = onCheckpoint;
            _checkpointSent = checkpointSent;
            ...
            {
                onItemsRemoved = (owningSource, count) => ((ParallelSource<T>)owningSource).OnItemsRemoved(count);
            ...
            _source = new SourceCore<T>(this, dataflowBlockOptions,
                owningSource => ((ParallelSource<T>)owningSource).Complete(),
                onItemsRemoved);
            ...
            {
                var thisBlock = ((ParallelSource<T>)state!) as IDataflowBlock;
            ...
            Common.WireCancellationToComplete(
                dataflowBlockOptions.CancellationToken, _source.Completion, owningSource => ((ParallelSource<T>)owningSource!).Complete(), this);
        ...
        internal void ConsumeAsyncIfNecessary(bool isReplacementReplica = false)
        {
                ...
                _boundingState.CountIsLessThanBound)
            {
                ...
                _boundingState.TaskForInputProcessing =
                    new Task(state => ((ParallelSource<T>)state!).ConsumeMessagesLoopCore(), this,
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (consumed)
                    {
                        ...
                        {
                            ProcessCheckpoint(consumedValue);
                        }
                        else
                        {
        ...
        private void ProcessCheckpoint(T checkpointEvent)
        {
            ...
            {
                var thisBufferBlock = (ParallelSource<T>)state!;
                return thisBufferBlock._onCheckpoint((checkpointEvent as ILockingEvent)!);
            }, this, CancellationToken.None, Common.GetCreationOptionsForTask(), TaskScheduler.Default)
                .Unwrap()
                .ContinueWith((task, state) =>
                {
                    var thisBlock = (ParallelSource<T>)state!;
                    lock (thisBlock.IncomingLock)
                    {
                        var outputEvent = (T)task.Result;
                        thisBlock._source.AddMessage(outputEvent);
                    }
                    // Notice that checkpoint has been sent downstream, it is now possible to open up for more message processing
                    _checkpointSent();
                }, this);
        }

        //private async Task ProcessCheckpointCore()
        //{
        //    await _onCheckpoint();
        ...
        private sealed class DebugView
        {
            ...
            private readonly ParallelSource<T> _bufferBlock;
            ...
            public DebugView(ParallelSource<T> bufferBlock)
            {
