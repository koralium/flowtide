...
namespace FlowtideDotNet.Base.Vertices.MultipleInput
{
    public class MultipleInputTarget<T> : ITargetBlock<T>, IReceivableSourceBlock<KeyValuePair<int, T>>, IDebuggerDisplay
    {
        ...
        public MultipleInputTarget(int targetId) :
            this(targetId, DataflowBlockOptionsExtensions.DefaultDataflowBlockOptions)
        { }
        ...
        public MultipleInputTarget(int targetId, DataflowBlockOptions dataflowBlockOptions)
        {
            _targetId = targetId;
            ...
            Action<ISourceBlock<KeyValuePair<int, T>>, int>? onItemsRemoved = null;
            if (dataflowBlockOptions.BoundedCapacity > 0)
            {
                onItemsRemoved = (owningSource, count) => ((MultipleInputTarget<T>)owningSource).OnItemsRemoved(count);
                _boundingState = new BoundingStateWithPostponedAndTask<T>(dataflowBlockOptions.BoundedCapacity);
            }

            // Initialize the source state
            _source = new SourceCore<KeyValuePair<int, T>>(this, dataflowBlockOptions,
                owningSource => ((MultipleInputTarget<T>)owningSource).Complete(),
                onItemsRemoved);
            ...
            {
                var thisBlock = ((MultipleInputTarget<T>)state!) as IDataflowBlock;
            ...
            Common.WireCancellationToComplete(
                dataflowBlockOptions.CancellationToken, _source.Completion, owningSource => ((MultipleInputTarget<T>)owningSource!).Complete(), this);
        ...
        private void TryHandleCheckpointEvent(T e)
        {
        ...
        DataflowMessageStatus ITargetBlock<T>.OfferMessage(DataflowMessageHeader messageHeader, T messageValue, ISourceBlock<T>? source, bool consumeToAccept)
        {
            ...
            lock (IncomingLock)
            {
                    ...
                    (_boundingState.CountIsLessThanBound && _boundingState.PostponedMessages.Count == 0 && _boundingState.TaskForInputProcessing == null && !_inCheckpoint))
                {
                    ...
                    TryHandleCheckpointEvent(messageValue);

                    _source.AddMessage(new KeyValuePair<int, T>(_targetId, messageValue!));
        ...
        public IDisposable LinkTo(ITargetBlock<KeyValuePair<int, T>> target, DataflowLinkOptions linkOptions) { return _source.LinkTo(target, linkOptions); }

        /// <include file='XmlDocs/CommonXmlDocComments.xml' path='CommonXmlDocComments/Sources/Member[@name="TryReceive"]/*' />
        public bool TryReceive(Predicate<KeyValuePair<int, T>>? filter, [MaybeNullWhen(false)] out KeyValuePair<int, T> item) { return _source.TryReceive(filter, out item); }

        /// <include file='XmlDocs/CommonXmlDocComments.xml' path='CommonXmlDocComments/Sources/Member[@name="TryReceiveAll"]/*' />
        public bool TryReceiveAll([NotNullWhen(true)] out IList<KeyValuePair<int, T>>? items) { return _source.TryReceiveAll(out items); }
        ...
        KeyValuePair<int, T> ISourceBlock<KeyValuePair<int, T>>.ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<KeyValuePair<int, T>> target, out bool messageConsumed)
        {
            return _source.ConsumeMessage(messageHeader, target, out messageConsumed);
        }

        /// <include file='XmlDocs/CommonXmlDocComments.xml' path='CommonXmlDocComments/Sources/Member[@name="ReserveMessage"]/*' />
        bool ISourceBlock<KeyValuePair<int, T>>.ReserveMessage(DataflowMessageHeader messageHeader, ITargetBlock<KeyValuePair<int, T>> target)
        {
            return _source.ReserveMessage(messageHeader, target);
        }

        /// <include file='XmlDocs/CommonXmlDocComments.xml' path='CommonXmlDocComments/Sources/Member[@name="ReleaseReservation"]/*' />
        void ISourceBlock<KeyValuePair<int, T>>.ReleaseReservation(DataflowMessageHeader messageHeader, ITargetBlock<KeyValuePair<int, T>> target)
        {
        ...
        internal void ConsumeAsyncIfNecessary(bool isReplacementReplica = false)
        {
                ...
                _boundingState.CountIsLessThanBound)
            {
                ...
                _boundingState.TaskForInputProcessing =
                    new Task(state => ((MultipleInputTarget<T>)state!).ConsumeMessagesLoopCore(), this,
                ...
                try
                {
                    ...
                    if (consumed)
                    {
                        if (consumedValue is ILockingEvent)
                        {
                            lock (IncomingLock)
                            {
                                TryHandleCheckpointEvent(consumedValue);
                            }
                        }

                        _source.AddMessage(new KeyValuePair<int, T>(_targetId, consumedValue!));
                        return true;
                    }
                }
                finally
        ...
        private void CompleteTargetIfPossible()
        {
                ...
                (_boundingState == null || _boundingState.TaskForInputProcessing == null))
            {
                ...
                if (_boundingState != null && _boundingState.PostponedMessages.Count > 0)
                {
                    ...
                    {
                        var thisBufferBlock = (MultipleInputTarget<T>)state!;
        ...
        private sealed class DebugView
        {
            ...
            private readonly MultipleInputTarget<T> _bufferBlock;
            /// <summary>The buffer's source half.</summary>
            private readonly SourceCore<KeyValuePair<int, T>>.DebuggingInformation _sourceDebuggingInformation;

            /// <summary>Initializes the debug view.</summary>
            /// <param name="bufferBlock">The BufferBlock being viewed.</param>
            public DebugView(MultipleInputTarget<T> bufferBlock)
            {
            ...
            public IEnumerable<KeyValuePair<int, T>> Queue { get { return _sourceDebuggingInformation.OutputQueue; } }
            ...
            public TargetRegistry<KeyValuePair<int, T>> LinkedTargets { get { return _sourceDebuggingInformation.LinkedTargets; } }
            /// <summary>Gets the set of all targets linked from this block.</summary>
            public ITargetBlock<KeyValuePair<int, T>>? NextMessageReservedFor { get { return _sourceDebuggingInformation.NextMessageReservedFor; } }
        }
    }
}
