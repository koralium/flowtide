...
namespace FlowtideDotNet.Base.Vertices.Unary
{
    public class ParallelUnaryTarget<T> : ITargetBlock<T>, IReceivableSourceBlock<T>, IDebuggerDisplay
    {
        ...
        public ParallelUnaryTarget() :
            this(DataflowBlockOptionsExtensions.DefaultDataflowBlockOptions)
        { }
        ...
        public ParallelUnaryTarget(DataflowBlockOptions dataflowBlockOptions)
        {
            ...
            {
                onItemsRemoved = (owningSource, count) => ((ParallelUnaryTarget<T>)owningSource).OnItemsRemoved(count);
            ...
            _source = new SourceCore<T>(this, dataflowBlockOptions,
                owningSource => ((ParallelUnaryTarget<T>)owningSource).Complete(),
                onItemsRemoved);
            ...
            {
                var thisBlock = ((ParallelUnaryTarget<T>)state!) as IDataflowBlock;
            ...
            Common.WireCancellationToComplete(
                dataflowBlockOptions.CancellationToken, _source.Completion, owningSource => ((ParallelUnaryTarget<T>)owningSource!).Complete(), this);
        ...
        private bool TryHandleCheckpointEvent(T e)
        {
            ...
            if (e is ILockingEvent checkpointEvent)
            {
                _inCheckpoint = true;
                return true;
            }
        ...
        internal void ConsumeAsyncIfNecessary(bool isReplacementReplica = false)
        {
                ...
                _boundingState.CountIsLessThanBound)
            {
                ...
                _boundingState.TaskForInputProcessing =
                    new Task(state => ((ParallelUnaryTarget<T>)state!).ConsumeMessagesLoopCore(), this,
        ...
        private void CompleteTargetIfPossible()
        {
                ...
                (_boundingState == null || _boundingState.TaskForInputProcessing == null))
            {
                ...
                if (_boundingState != null && _boundingState.PostponedMessages.Count > 0)
                {
                    ...
                    {
                        var thisBufferBlock = (ParallelUnaryTarget<T>)state!;
        ...
        private sealed class DebugView
        {
            ...
            private readonly ParallelUnaryTarget<T> _bufferBlock;
            ...
            public DebugView(ParallelUnaryTarget<T> bufferBlock)
            {
