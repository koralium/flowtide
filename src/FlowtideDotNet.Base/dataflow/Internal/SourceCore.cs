...
namespace DataflowStream.dataflow.Internal
{
    ...
    internal sealed class SourceCore<TOutput>
    {
        ...
        internal TOutput? ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<TOutput> target, out bool messageConsumed)
        {
            ...
            TOutput? consumedMessageValue = default;
        ...
        internal void ReleaseReservation(DataflowMessageHeader messageHeader, ITargetBlock<TOutput> target)
        {
            ...
            lock (OutgoingLock)
            {
                ...
                if (_nextMessageReservedFor != target) throw new InvalidOperationException("Message not reserved by target");

                lock (ValueLock)
                {
                    // If this is not the message at the head of the queue, bail
                    if (messageHeader.Id != _nextMessageId.Value || _messages.IsEmpty) throw new InvalidOperationException("Message not reserved by target");
        ...
        internal bool TryReceiveAll([NotNullWhen(true)] out IList<TOutput>? items)
        {
            ...
            if (countReceived > 0)
            {
                ...
                if (_itemsRemovedAction != null)
                {
                    int count = _itemCountingFunc != null ? _itemCountingFunc(_owningSource, default!, items) : countReceived;
                    _itemsRemovedAction(_owningSource, count);
                }
        ...
        private bool OfferToTargets(ITargetBlock<TOutput>? linkToTarget = null)
        {
            ...
            DataflowMessageHeader header = default;
            TOutput? message;
        ...
        private void OfferMessagesLoopCore()
        {
            ...
            try
            {
                int maxMessagesPerTask = _dataflowBlockOptions.GetActualMaxMessagesPerTask();
        ...
        private void CompleteBlockOncePossible()
        {
            ...
            {
                _completionTask.TrySetResult(default);
            }
