...
{
    internal class BooleanEncoder : IColumnEncoder
    {
        public Task AddValue(Dictionary<string, object> obj, string columnName, FlxValue flxValue)
        {
            ...
            {
                var val = flxValue.AsBool;
                obj[columnName] = val;
            }
            else if (flxValue.ValueType == FlexBuffers.Type.Int)
            {
                var val = flxValue.AsLong;
                obj[columnName] = val != 0;
            }
            else if (flxValue.ValueType == FlexBuffers.Type.String)
            {
                var val = flxValue.AsString;
                obj[columnName] = val == "1" || val == "true";
            }
            else
            {
                throw new NotSupportedException("Boolean columns should get boolean values only");
            }
