...
*   **Stream Version Management:** Stream versioning is opt-in and requires explicit configuration on the stream builder using `AddVersioningFromPlanHash()`, `AddVersioningFromString()`, or `AddVersioningFromAssembly()`. These can be combined; a new version is created whenever any of the configured sources change. Without versioning, the stream always uses a single `"default"` version and version cleanup does not apply. When versioning is enabled, each version stores its state in a separate directory, and multiple versions can coexist in storage. The `KeepLastStreamVersions` setting (default -1, keep all) controls automatic cleanup: after each checkpoint, versions are sorted by their last initialization time and the oldest versions beyond the retention count are deleted. The current version is always preserved; the setting specifies how many *previous* versions to retain alongside it. For example, a value of 0 deletes all old versions immediately, and a value of 1 keeps one previous version.
