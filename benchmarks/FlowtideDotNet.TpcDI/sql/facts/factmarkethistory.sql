...
SELECT
  ds.SK_SecurityID,
  ds.SK_CompanyID,
  CAST(timestamp_format(mh.DM_DATE, 'yyyyMMdd') as INT) as SK_DateID,
  mh.ClosePrice / YEAR_FI_BASIC_EPS as PERatio,
  mh.DM_S_SYMB,
  mh.YearHigh.high as FiftyTwoWeekHigh,
  CAST(timestamp_format(mh.YearHigh.date, 'yyyyMMdd') as INT) as SK_FiftyTwoWeekHighDate,
  mh.YearLow.low as FiftyTwoWeekLow,
  CAST(timestamp_format(mh.YearLow.date, 'yyyyMMdd') as INT) as SK_FiftyTwoWeekLowDate,
  ClosePrice,
  DayHigh,
  DayLow,
  Volume
