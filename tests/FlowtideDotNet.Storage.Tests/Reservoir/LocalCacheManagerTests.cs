...
namespace FlowtideDotNet.Storage.Tests.Reservoir
{
    public class LocalCacheManagerTests
    {
        ...
        private ReservoirPersistentStorage blobPersistentStorage;
        public LocalCacheManagerTests()
        {
            persistentData = new TestDataProvider();
            cacheData = new TestDataProvider();
            blobPersistentStorage = new ReservoirPersistentStorage(new Persistence.Reservoir.ReservoirStorageOptions()
            {
        ...
        public async Task EnsureDataInCacheIsReinitializedAfterCrash()
        {
            ...
            {
                var newStorage = new ReservoirPersistentStorage(new Persistence.Reservoir.ReservoirStorageOptions()
                {
                    FileProvider = persistentData,
                    CacheProvider = cacheData
                });
